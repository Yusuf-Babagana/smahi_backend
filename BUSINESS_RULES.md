...
| **Embedded `_details` payloads slimmed (2026-07-17):** `country_details`/`state_details` embedded in users, bookings, and artisan profiles now use the Lite serializers (id/name/code) instead of nesting the full location tree (37 states × their LGAs ≈ 70KB per user, 1.2MB per artisan page → 3.5KB). Verified: the mobile app reads only `id`/`name` from these. The standalone `/api/locations/` endpoints keep full nested shapes | ✅ |
| **GPS coordinates (wired 2026-07-17):** `User.latitude/longitude` are now self-service via profile PATCH (validated ±90/±180). The app auto-syncs them: artisan → on dashboard open; client → on home open. They power distance search (`?latitude&longitude`, `?use_saved=true`, `max_distance`) and "X km away" labels. ⚠️ Privacy: `UserSerializer` now exposes coords in every `_details` embed (alongside the already-flagged email/phone) — revisit with the D-series serializer-exposure decision | ✅ |
